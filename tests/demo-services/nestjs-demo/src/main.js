 ...
 *   1. call `createAuthz(...)` with your trust-root URLs + authorization.yaml,
 ...
 *
 * `createAuthz` does all the wiring for you: it compiles authorization.yaml
 * (fail-fast), validates user JWTs and service tokens against their JWKS, runs
 * the startup state machine (Role Service snapshot -> disk-seed fallback ->
 * Kafka subscribe), and emits an audit event per decision.
 *
...
async function main() {
  const authz = await createAuthz({
    ...
    // Outbound identity: this service's own token, acquired via client-credentials.
    serviceToken: {
